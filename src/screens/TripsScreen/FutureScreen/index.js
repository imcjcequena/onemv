...
class FutureScreen extends Component {
  ...
  render = () => {
    ...
    return (passengerId
      && (
        <Query url={`/passengers/${passengerId}/trips?filter[schedule]=future&sort=trip_date,pickup.start_window`}>
          {
            ({
              loading,
              error,
              ...
              if (loading) {
                return (
                  MVStore.futureTrips ? MVStore.futureTrips.map(
                  ...
                  <div className="h-full flex items-center justify-center text-center">
                    <Card className="h-32">
                      <div className="h-full font-bold flex items-center justify-center text-center">
                        <Text text="YOU_HAVE_NO_FUTURE_RESERVATIONS" />
                      </div>
                    </Card>
                  </div>
                );
              }
              MVStore.setFutureTrips(trips);
...
export default compose(inject('MVStore'), observer)(withRouter(FutureScreen));
